...
namespace UnityEngine.XR.ARKit
{
    ...
    /// </summary>
    [Obsolete("ARKitXRPlaneSubsystem has been deprecated in Apple ARKit XR Plug-in 6.0. Use ARKitPlaneSubsystem instead (UnityUpgradable) -> UnityEngine.XR.ARKit.ARKitPlaneSubsystem", true)]
        ...
        class ARKitProvider : Provider
        {
                ...
                ref NativeArray<Vector2> boundary)
            {
                ...
                try
                {
                    ...
                    var transformPositionsHandle = new TransformBoundaryPositionsJob
                    {
                        ...
                        inverseYAxisRotation = Quaternion.Inverse(rotation)
            ...
            struct TransformBoundaryPositionsJob : IJobParallelFor
            {
                ...
                public void Execute(int index)
                {
                    ...
                    // we also compensate for ARKit rotationOnYAxis, if the build target is iOS 16.0 or greater.
                    // It's necessary to do this here since we can't work with arkit rotations due to handedness.
                    // so we flip the signs first then rotate "back" into the correct place.
